...
class Script(QObject):
    ...
    def apply_img2img(self, is_inpaint, is_upscale=False):
        ...
        mask_trigger = self.transparency_mask_inserter()
        mask_image = self.get_mask_image(use_official_api) if is_inpaint else None
        glayer = self.doc.createGroupLayer("Unnamed Group")
        self.doc.rootNode().addChildNode(glayer, None)
        insert = self.img_inserter(
        ...
        if is_inpaint and mask_image is not None:
            ...
            self.official_api_transparency_mask_inserter(glayer, mask_image)
            self.doc.refreshProjection()
    ...
    def official_api_transparency_mask_inserter(self, glayer, mask_image):
